...
function ClickTitle() {
    document.querySelector("#favourites_box").style.right = "-100%";
    Router.push("/Recipe");
}

function ClickFav() {
    document.querySelector("#favourites_header").style.right = "-100%";
...
const FavouritesPage = ({}) => {
    return <div id="favourites_page">
        <BackHeader/>
        <div id="favourites_header"  onClick={ClickFav}>
            <div id="favourite-star-icon"><FaStar size="0.75em" color="#FFF"/></div>
            <Header text="Favourites" fontSize={12}/>
        </div>

        <div id="favourites_box" onClick={ClickTitle}>
            <FavouritesBox text="Savoury Breakfast Bowl" />
